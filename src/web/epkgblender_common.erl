...
notification_area() ->
    case (wf:page_module() == epkgblender_validate_email) or
...
event(resend_validation_email) ->
    {bad_validation_token, User} = epkgblender_user_server:validate_email(wf:user(), "dummy-token"),
...
send_validation_email(User) ->
    epkgblender_utils:send_email(User#epkgblender_user.name, User#epkgblender_user.email, "Validate Your E-mail Address",
        hd(string:tokens(User#epkgblender_user.name, " ")) ++ ",\n"
        "\n"
        "Before you can continue using pkgblender, you must validate your e-mail\n"
        "address by clicking the following link:\n"
        "\n"
        "    http://" ++ wf:header(host) ++ "/validate_email?token=" ++ User#epkgblender_user.validation_token ++ "\n"
        "\n"
