%%%
%%% epkgblender_validate_email.erl
...
-module(epkgblender_validate_email).
-compile(export_all).
...
content() ->
    case epkgblender_user_server:validate_email(wf:user(), wf:q(token)) of
        {ok, User} ->
            lists:foreach(fun(Role) -> wf:role(Role, true) end, User#epkgblender_user.roles),
